...
public class Page 
{
	...
	public String getPaging()
	{
		...
		{
			paging += "<a href=\"javascript:fncGetList('" + (getCurrentPage()-1) + "')\">◀ 이전</a>";
		}
		...
		for(int i = getBeginUnitPage(); i<= getEndUnitPage() ;i++)
		{
			if(i == getCurrentPage())
	    	{ 
				paging += "<a href=\"javascript:fncGetList('"+ i +"');\" style='color:red; font-size:15px;'>";
				paging += "<b> "+ i +" </b>";
				paging += "</a>";
	    	}else
	    	{ 
	    		paging += "<a href=\"javascript:fncGetList('"+ i +"');\">";
		...
		{
				paging += "<a href=\"javascript:fncGetList('"+ (getEndUnitPage()+1) +"')\">이후 ▶</a>";
		}
